...
nodes <- read.csv("u-nodes.csv")
links <- read.csv("u-edges.csv")
...
V(net)$label <- ifelse(V(net)$type=='miRNA' | V(net)$name=='MTOR' |
                         ...
                         V(net)$name=='RPTR' | V(net)$name=='PAK7'|
                         V(net)$name=='ARHGEF3', V(net)$name, NA)
...
#Vdist <- ifelse(V(net)$type == 'mRNA', 0, ifelse(V(net)$name == 'ARHGEF3', 0, 0))
V.label.size <- ifelse(V(net)$type == 'mRNA', 1, 1.2)
...
#tiff(filename='GFPup-miRNA.tiff', 14*300, 14*300, res=300)
